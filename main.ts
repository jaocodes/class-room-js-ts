...
class PersistentInMemoryDbProtected extends ImMemoryDatabaseProtected implements Persistable{
  saveToString(): string{
...
console.log(`Informação da chave foo após a primeira adição: ${myDb3.get('foo')}`)

myDb3.set('foo', 'eps')

console.log(`Informação da chave foo após a segunda adição: ${myDb3.get('foo')}`)
...
console.log(`Informação da chave foo após a recuperar o estado: ${myDb3.get('foo')}`)